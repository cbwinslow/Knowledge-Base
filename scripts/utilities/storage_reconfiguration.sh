...
# Check available space (in extents)
FREE_EXTENTS=$(vgdisplay ubuntu-vg | grep "Free PE / Size" | awk '{print $5}')
echo "Free extents available: $FREE_EXTENTS"

# Calculate sizes (using approximately 60% for data, 30% for backup, 10% reserved)
TOTAL_EXTENTS=$(vgdisplay ubuntu-vg | grep "Total PE" | awk '{print $3}')
DATA_EXTENTS=$((TOTAL_EXTENTS * 60 / 100))
BACKUP_EXTENTS=$((TOTAL_EXTENTS * 30 / 100))
