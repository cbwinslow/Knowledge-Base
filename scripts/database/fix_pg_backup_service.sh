#!/bin/bash
# Script to fix PostgreSQL backup service

echo "Creating and installing PostgreSQL backup script..."

# Create the backup script
cat > /tmp/cbw-pg-backup.sh << 'EOF'
#!/bin/bash
...
# Make it executable
chmod +x /tmp/cbw-pg-backup.sh

echo "Backup script created at /tmp/cbw-pg-backup.sh"
echo "To install it, run with sudo:"
echo "  sudo mv /tmp/cbw-pg-backup.sh /usr/local/sbin/cbw-pg-backup.sh"
echo "  sudo systemctl daemon-reload"
echo "  sudo systemctl start cbw-pg-backup.service"