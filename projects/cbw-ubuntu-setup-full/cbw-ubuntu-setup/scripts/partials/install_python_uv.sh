...
if [[ ! -d "/usr/local/pyenv" ]]; then git clone https://github.com/pyenv/pyenv.git /usr/local/pyenv; fi
export PYENV_ROOT="/usr/local/pyenv"; export PATH="$PYENV_ROOT/bin:$PATH"; eval "$(pyenv init -)"
pyenv install -s 3.10.14; pyenv global 3.10.14
