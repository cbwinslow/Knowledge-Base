#!/usr/bin/env bash
set -euo pipefail
PG_VER="${PG_VER:-16}"; APP_DB="${APP_DB:-app}"; APP_USER="${APP_USER:-appuser}"; APP_PASS="${APP_PASS:-apppass}"
...
PG_CONF="/etc/postgresql/${PG_VER}/main/postgresql.conf"; PG_HBA="/etc/postgresql/${PG_VER}/main/pg_hba.conf"
if [ -f "$PG_CONF" ]; then
