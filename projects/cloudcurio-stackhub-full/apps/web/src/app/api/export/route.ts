...
export async function GET(req: NextRequest){
  ...
  const items = (seed.items as any[]).filter(i => ids.includes(i.id))
  ...
  return new NextResponse(content, { headers:{ 'Content-Type':'text/plain; charset=utf-8', 'Content-Disposition':`attachment; filename=${filename}` } })
}
